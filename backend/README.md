...
# Video Upload Module — Drop-in for CodeVerse Backend


```
video-upload-module/
├── src/
│   ├── controllers/videoController.js   ← upload + stream logic
│   ├── middleware/videoUpload.js         ← multer config
│   ├── models/InterviewRecording.js      ← Mongoose schema
│   └── routes/videoRoutes.js             ← Express routes
...
|--------|------|------|-------------|
| POST | `/api/upload-interview-video` | JWT | Upload webm from frontend |
| GET | `/api/upload-interview-video/:sessionId` | Admin | Fetch recording metadata |
| GET | `/api/upload-interview-video/:sessionId/stream` | Admin | Stream/download video |
...
```
backend/uploads/interview-videos/interview_{sessionId}_{timestamp}.webm
```
...
## Frontend already sends this — nothing to change

```js
const form = new FormData()
form.append('video', blob, 'interview-recording.webm')
